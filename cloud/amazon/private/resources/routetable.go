...
func (r *RouteTable) Apply(actual, expected cloud.Resource, immutable *cluster.Cluster) (*cluster.Cluster, cloud.Resource, error) {
	...
	if lsn != 3 {
		return nil, nil, fmt.Errorf("Found [%d] NAT Gateways for ID [%s]", lsn, r.ServerPool.Identifier)
	}
	for _, ig := range output.NatGateways {
		logger.Info("Mapping route table [%s] to NAT gateway [%s]", *rtOutput.RouteTable.RouteTableId, *ig.NatGatewayId)

		// --- Map Route Table to NAT Gateway
		riInput := &ec2.CreateRouteInput{
			DestinationCidrBlock: S("0.0.0.0/0"),
			GatewayId:            ig.NatGatewayId,
